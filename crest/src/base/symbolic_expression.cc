...
namespace crest {
    ...
    void SymbolicExpr::AppendToString(string* s, int tp) const {
        char buff[32];
#ifdef PRINT_FOR_TOOL
        s->append("{");
        if (tp == 'i')
            sprintf(buff, "const=%lld", const_);
        else if (tp == 'c')
            sprintf(buff, "const=%d", (char) const_);
        else
            sprintf(buff, "const=%lld", const_);
        s->append(buff);
        ...
        for (ConstIt i = coeff_.begin(); i != coeff_.end(); ++i) {
            ...
            if (tp == 'i')
                ...
                sprintf(buff, ", x%u=%d", i->first, (char)i->second);
            else
                sprintf(buff, ", x%u=%lld", i->first, i->second);
            s->append(buff);
